...
// Helper: Split content into semantic chunks (server-side safe)
function splitIntoChunks(htmlContent, maxChunkSize = 12000) {
  // Strategy: Split at heading tags to keep semantic sections intact
  const headingPattern = /(<h[1-4][^>]*>.*?<\/h[1-4]>)/is;
  
  const chunks = [];
  let currentChunk = '';
  let remainingContent = htmlContent;
  
  // Keep splitting until we've processed all content
  while (remainingContent.length > 0) {
    // If remaining content is small enough, add it as final chunk
    if (remainingContent.length <= maxChunkSize) {
      if (currentChunk) {
        chunks.push(currentChunk + remainingContent);
      } else {
    ...
    // Find next heading within reasonable range
    const searchSpace = remainingContent.substring(0, maxChunkSize + 2000);
    const headingMatch = searchSpace.match(headingPattern);
    
    if (headingMatch && headingMatch.index > 0) {
      // Split at the heading
      const splitPoint = headingMatch.index;
      const chunkToAdd = remainingContent.substring(0, splitPoint);
      ...
      // No heading found, split at paragraph or just at max size
      const paragraphPattern = /<\/p>/i;
      const paragraphMatch = searchSpace.match(paragraphPattern);
      
      if (paragraphMatch && paragraphMatch.index > 0) {
        const splitPoint = paragraphMatch.index + 4; // Include </p>
        currentChunk += remainingContent.substring(0, splitPoint);
        chunks.push(currentChunk);
        currentChunk = '';
        remainingContent = remainingContent.substring(splitPoint);
      } else {
        // No good split point, just chunk at max size
        currentChunk += remainingContent.substring(0, maxChunkSize);
        chunks.push(currentChunk);
        currentChunk = '';
        remainingContent = remainingContent.substring(maxChunkSize);
      }
    }
  }
  
  // Safety check: if no chunks created, return original content
